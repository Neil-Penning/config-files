...
#Bash files
    if [[ -e  $HOME/.bash_prompt ]]; then
        print_no_overwrite "$HOME/.bash_prompt"
    else
...
# Vim 
    ...
    if [[ -e ./vim/vim-tex-syntax/syntax/tex.vim ]]; then
        ...
        print_dne "\$CONFIG_FILES/vim/vim-tex-syntax/"
    fi

# SSH
    if [[ -f $HOME/.ssh/config ]]; then
        print_no_overwrite "$HOME/.ssh/config"
    else
...
# Tmux
    ...
    if [[ -e  $HOME/.tmux_profile.conf ]]; then
        print_no_overwrite "$HOME/.tmux_profile.conf"
    else
